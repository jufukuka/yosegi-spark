...
public class SparkSequentialByteLoader extends AbstractSparkSequentialNumberLoader {
  ...
  public SparkSequentialByteLoader(WritableColumnVector vector, int loadSize, int totalChildCount, int currentChildCount) {
  ...
  public void setByte(int index, byte value) throws IOException {
    vector.putByte(getIndex(index), value);
  }
}
