...
public class SparkColumnarBatchConverter implements IRawConverter<ColumnarBatch> {
  ...
  @Override
  public ColumnarBatch convert(List<ColumnBinary> raw, int loadSize) throws IOException {
    // NOTE: initialize
    ...
    for (int i = 0; i < raw.size(); i++) {
      ColumnBinary columnBinary = raw.get(i);
      if (!keyIndexMap.containsKey(columnBinary.columnName)) {
        continue;
      }
      int index = keyIndexMap.get(columnBinary.columnName);
      isSet[index] = true;
      SparkLoaderFactoryUtil.createLoaderFactory(childColumns[i]).create(columnBinary, loadSize);
    }
    // NOTE: null columns
    for (int i = 0; i < childColumns.length; i++) {
      if (!isSet[i]) {
    ...
    WritableColumnVector[] partColumns =
